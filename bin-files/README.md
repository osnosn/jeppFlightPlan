...
Compiled by golang-1.16.   
 ...
 | v-1.2 |  |
 |--------|-------|
 | [aarch64-linux](aarch64-linux-jeppFlightPlan.zip) | arm 64bit linux 命令行的可执行文件 |
 |   [amd64-linux](amd64-linux-jeppFlightPlan.zip)   | x86 64bit linux 命令行的可执行文件 |
 |         [win32](win32-jeppFlightPlan.exe.zip)  | windows 32bit 命令行的可执行文件 |
 |         [win64](win64-jeppFlightPlan.exe.zip)  | windows 64bit 命令行的可执行文件 |
...
------JeppFD-Pro 或 FliteDeckPro 配置说明----
 ...
 在 ipad 自带的系统设置 "设置" -> 在左侧找到 "FD Pro" 或 "FD Pro X" -> ACCOUNT INFO "Services"
  ...
  打开 FliteDeckPro , 
  找到 "Flight Plans" ,在"Call sign"位置,输入icao或iata机场对，就能看到航路计划了。
